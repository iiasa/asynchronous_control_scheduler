...
6. `final_dimensions_order` list of all dimensions including both columns present in the input csv timeseries dataset and [extra dimension](#extra_dimension) presented by template itself. During validation, if the extra dimension value is already present then it is replaced. 


## Extra Dimension
1. Extra dimensions are referred to the dimension not defined explicitly as properties in root json schema. Instead, directly mapping object is provided with the key `map_<dimension_name>` in schema json.